...
- ~~코드 구문 하이라이팅 — 현재 없음. 나중에 독립적으로 추가 가능~~
  **2026-08-31 정정: 이 서술이 틀렸다.** 확인하지 않고 단정했다. 실제로는 CMS 에디터가
  Prism으로 색칠한 결과를 HTML에 저장해뒀고(토큰 span 2703개), `app/globals.css`에
  대응하는 Prism 테마가 들어 있었다. 색칠이 **저장된 마크업에** 있어서 눈에 띄지 않았을 뿐,
  기존 사이트의 코드에는 색이 있었다. md로 옮기며 그 span이 평문으로 펴져 색이 사라졌으므로
  `prismjs`를 렌더 단계에 넣어 복구했다 (`app/lib/posts/render.ts`). 범위 제외가 아니라
  **기존 동작 보존** 항목이었다.
